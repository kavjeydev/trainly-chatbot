...
export default function EmbedChat() {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data.type === 'TRAINLY_TOGGLE') {
        setIsMinimized(prev => !prev);
      } else if (event.data.type === 'TRAINLY_OPEN') {
        setIsMinimized(false);
      } else if (event.data.type === 'TRAINLY_CLOSE') {
        setIsMinimized(true);
      }
    };
  ...
  useEffect(() => {
    window.parent.postMessage({ type: 'TRAINLY_STATE', isMinimized }, '*');
  }, [isMinimized]);
  ...
  return (
    <div className="fixed inset-0 flex items-end justify-end p-4 pointer-events-none font-sans">
      {/* Chat Button */}
      <button
        onClick={() => setIsMinimized(!isMinimized)}
        className={`pointer-events-auto w-14 h-14 rounded-full bg-blue-500 flex items-center justify-center shadow-lg hover:bg-blue-600 transition-all duration-300 ${!isMinimized ? 'scale-0 opacity-0' : 'scale-100 opacity-100'}`}
      ...
      {isMinimized && (
        <div
          className="absolute bottom-4 right-4 w-14 h-14 rounded-full animate-ping pointer-events-none"
      ...
      {/* Chat Window */}
      <div
        className={`pointer-events-auto w-[380px] h-[520px] bg-[#0a0a0a] border border-amber-500/20 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 origin-bottom-right ${!isMinimized ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-amber-500/10 bg-black/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">{branding.chatbotName}</h3>
